//
// Created by Rainy Memory on 2021/1/16.
//
...
#ifdef circle
    for (int i = 1; i < 8; i++) {
        path = "../Data/AdvancedDataSet/testcase5/";
        path += (char) (i + '0');
        path += ".in";
        testfs.open(path, ios::in);
        while (getline(testfs, cmd)) {
            try {
                runCommand(cmd);
...
#endif
#ifndef circle
    path = "../Data/AdvancedDataSet/testcase5/1.in";
    testfs.open(path, ios::in);
    while (getline(testfs, cmd)) {
        try {
            runCommand(cmd);
